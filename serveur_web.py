...
def get_produits():
    state = load_state()
    return jsonify(state.get("produits", []))
...
def save_produits():
    ...
    state = load_state()
    state["produits"] = data.get("produits", [])
    save_state(state)
...
def get_livreurs():
    state = load_state()
    return jsonify({
        "livreurs": state.get("livreurs", []),
        "zone": state.get("zone", "Nancy centre · Maxéville · Laxou · Vandœuvre · Essey-lès-Nancy")
    })
...
def save_livreurs():
    ...
    if uid not in ADMIN_IDS:
        return jsonify({"error": "Non autorisé"}), 403
    state = load_state()
    state["livreurs"] = data.get("livreurs", [])
    state["zone"] = data.get("zone", "")
    save_state(state)
    return jsonify({"ok": True})
