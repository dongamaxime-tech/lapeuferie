...
function applyAllData(d) {
  ...
  if(d.livreurs && d.livreurs.length > 0) livreurs = d.livreurs;
  if(d.zone) zoneText = d.zone;
  if(d.statut) curStatut = d.statut;
  if(Array.isArray(d.loyalty)) {
    loyaltyTiers = d.loyalty;
    renderLoyaltyClient();
    renderAdminLoyalty();
  }
  if(typeof d.whatsapp === 'string') whatsappNumber = d.whatsapp;
  if(Array.isArray(d.loader_msgs) && d.loader_msgs.length > 0) loaderMessages = d.loader_msgs;
  renderAll();
  renderLivreurs();
  renderAdminLivreurs();
  renderStatut();
...
function loadData() {
  showLoader();
  var CACHE_KEY = 'pufferie_cache_v1';

  // Affichage immédiat depuis le cache localStorage
  try {
    var cached = localStorage.getItem(CACHE_KEY);
    if(cached) {
      applyAllData(JSON.parse(cached));
      hideLoader(); // masquer le sablier immédiatement si cache dispo
    }
  } catch(e) {}

  // Rafraîchissement en arrière-plan depuis Supabase
  apiGet('/api/all').then(function(data) {
    try { localStorage.setItem(CACHE_KEY, JSON.stringify(data)); } catch(e) {}
    applyAllData(data);
    hideLoader();
    // Init messages loader si Supabase vide
