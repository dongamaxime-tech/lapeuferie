...
@keyframes fadeMsg { 0%,100%{opacity:0;transform:translateY(6px)} 15%,85%{opacity:1;transform:translateY(0)} }
#loader-spinner { animation: spin 1s linear infinite; }
#loader-msg-wrap { animation: fadeMsg 4s ease-in-out infinite; }
</style>
<div id="loading-screen" style="display:none;position:fixed;inset:0;z-index:5000;background:var(--bg);flex-direction:column;align-items:center;justify-content:center;gap:32px;transition:opacity 0.4s ease">
  ...
  <!-- Spinner -->
  <svg id="loader-spinner" width="64" height="64" viewBox="0 0 64 64" fill="none">
    <circle cx="32" cy="32" r="26" stroke="rgba(255,255,255,0.1)" stroke-width="5"/>
    <path d="M32 6 A26 26 0 0 1 58 32" stroke="url(#spg)" stroke-width="5" stroke-linecap="round"/>
    <defs>
  ...
  <!-- Messages rotatifs configurables admin -->
  <div id="loader-msg-wrap" style="text-align:center;padding:0 32px;max-width:300px">
    <div id="loader-msg" style="font-size:15px;color:var(--text);font-weight:600;margin-bottom:6px;min-height:22px"></div>
    <div id="loader-sub" style="font-size:12px;color:var(--muted);line-height:1.6;min-height:36px"></div>
  </div>

  <div style="font-size:11px;color:var(--muted);opacity:0.5">Chargement du stock en cours…</div>
</div>
...
function showLoader() {
  ...
  var wrap = document.getElementById('loader-msg-wrap');
  var idx = 0;
  ...
  function showMsg() {
    ...
    if(!msgs || !msgs.length) {
      ...
      if(subEl) subEl.textContent = 'Le stock arrive dans quelques instants.';
      return;
    }
    var m = msgs[idx % msgs.length];
    if(wrap) { wrap.style.animation = 'none'; wrap.offsetHeight; wrap.style.animation = 'fadeMsg 4s ease-in-out infinite'; }
    if(msgEl) msgEl.textContent = m.title || '';
    if(subEl) subEl.textContent = m.sub || '';
    idx++;
  }

  showMsg();
  _msgTimer = setInterval(showMsg, 4000);
}
...
async function kvGet(key, env) {
  try {
    const val = await env.KV.get(key, { type: "json" });
    if (val !== null) return val;
  } catch(e) {}
  return await sbGet(key, env.SUPABASE_KEY);
}

async function kvSet(key, value, env) {
  try { await env.KV.put(key, JSON.stringify(value)); } catch(e) {}
  sbSet(key, value, env.SUPABASE_KEY).catch(() => {});
  return true;
}
...
export default {
  async fetch(request, env) {
    ...
    if (path === "/api/statut" && request.method === "POST") {
      ...
      await kvSet("statut", data.statut || "dispo", env);
      return cors({ ok: true });
    }
    ...
    if (path === "/api/produits" && request.method === "POST") {
      const data = await request.json();
      const ok = await kvSet("produits", data.produits || [], env);
      console.log(`Save produits: ${data.produits?.length} produits`);
      if (ok) return cors({ ok: true, count: data.produits?.length });
      return cors({ ok: false, error: "KV save failed" }, 500);
    }
    ...
    if (path === "/api/livreurs" && request.method === "POST") {
      const data = await request.json();
      await Promise.all([
        kvSet("livreurs", data.livreurs || [], env),
        kvSet("zone", data.zone || "", env)
      ]);
      return cors({ ok: true });
    }
    ...
    if (path === "/api/loyalty" && request.method === "POST") {
      const data = await request.json();
      await kvSet("loyalty", data.tiers || [], env);
      return cors({ ok: true });
    }
    ...
    if (path === "/api/whatsapp" && request.method === "POST") {
      const data = await request.json();
      await kvSet("whatsapp", data.number || "", env);
      return cors({ ok: true });
    }
    ...
    if (path === "/api/loader_msgs" && request.method === "POST") {
      const data = await request.json();
      await kvSet("loader_msgs", data.msgs || [], env);
      return cors({ ok: true });
    }
