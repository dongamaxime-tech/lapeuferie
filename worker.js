...
<!-- LOADING SCREEN -->
<div id="loading-screen" style="display:none;position:fixed;inset:0;z-index:5000;background:var(--bg);flex-direction:column;align-items:center;justify-content:center;transition:opacity 0.4s ease">
  <div style="margin-bottom:28px;text-align:center">
    <div style="font-family:'Bebas Neue',sans-serif;font-size:36px;letter-spacing:0.08em"><span style="background:linear-gradient(135deg,#ff6b6b,#ffd93d,#6bcb77,#4d96ff,#c77dff);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text">Pufferie</span> <span style="color:#fff">Nancy</span></div>
  </div>

  <!-- Cercle de décompte -->
  <div style="position:relative;width:110px;height:110px;margin-bottom:24px">
    <svg width="110" height="110" style="transform:rotate(-90deg)">
      <circle cx="55" cy="55" r="48" fill="none" stroke="rgba(255,255,255,0.08)" stroke-width="6"/>
      <circle id="countdown-ring" cx="55" cy="55" r="48" fill="none" stroke="url(#cg)" stroke-width="6" stroke-linecap="round"
        stroke-dasharray="301.6" stroke-dashoffset="0" style="transition:stroke-dashoffset 1s linear"/>
      <defs>
    ...
    <div style="position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center">
      <div id="countdown-num" style="font-family:'Bebas Neue',sans-serif;font-size:38px;color:#fff;line-height:1">23</div>
      <div style="font-size:10px;color:var(--muted);letter-spacing:0.15em">SEC</div>
    </div>
  </div>

  <div style="font-size:14px;color:var(--text);font-weight:500;margin-bottom:16px">Chargement du stock...</div>
  
  <!-- Messages rotatifs admin -->
  <div style="max-width:280px;text-align:center;padding:0 20px">
    <div id="loader-msg" style="font-size:13px;color:var(--p5);font-weight:600;margin-bottom:4px;transition:opacity 0.5s ease;min-height:20px"></div>
    <div id="loader-sub" style="font-size:11px;color:var(--muted);line-height:1.6;transition:opacity 0.5s ease;min-height:32px"></div>
  </div>
</div>
...
function showLoader() {
  // Plus de décompte : le worker Cloudflare répond instantanément, aucun écran de chargement n'est affiché.
  return;
}
...
function loadData() {
  showLoader();
  apiGet('/api/produits').then(function(data) {
    if(Array.isArray(data)) {
      products = data;
      nextId = data.length > 0 ? Math.max.apply(null, products.map(function(p){ return p.id||0; })) + 1 : 1;
    }
    renderAll();
  }).catch(function(e) {
    renderAll();
  ...
  // Charger livreurs + zone
  apiGet('/api/livreurs').then(function(data) {
  ...
  // Charger statut
  apiGet('/api/statut').then(function(data) {
    if(data.statut) curStatut = data.statut;
    renderStatut();
  }).catch(function() { renderStatut(); });

  // Charger programme fidélité
  loadLoyalty();
  loadWhatsapp();
  loadLoaderMsgs();
  // Sauvegarder les messages par défaut si Supabase est vide (premier lancement)
  apiGet('/api/loader_msgs').then(function(d) {
    if(Array.isArray(d) && d.length === 0) {
      // Initialiser Supabase avec les messages par défaut
